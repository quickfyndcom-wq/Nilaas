...
import logoSrc from "../assets/logo/Asset 7.png";
import { getPublicOrderNumber } from "@/lib/orderNumber";

// Optional: read company details from public env vars (client-safe)
const COMPANY_NAME = process.env.NEXT_PUBLIC_INVOICE_COMPANY_NAME || "Nilaas";
const COMPANY_ADDRESS_LINE1 = process.env.NEXT_PUBLIC_INVOICE_ADDRESS_LINE1 || "kerala, India";
const COMPANY_ADDRESS_LINE2 = process.env.NEXT_PUBLIC_INVOICE_ADDRESS_LINE2 || "";
const COMPANY_CONTACT = process.env.NEXT_PUBLIC_INVOICE_CONTACT || "Email: support@nilaas.in | Web: https://nilaas.in";
...
// Font config for proper INR rendering and better Unicode support
...
export const generateInvoice = async (order) => {
    ...
    // Header: Invoice title + number
        const orderIdShort = getPublicOrderNumber(order) || String(order?.id || order?._id || '').slice(-8);
        doc.setFont(UNICODE_FONT_NAME, 'bold');
        doc.setFontSize(18);
...
export const downloadInvoice = async (order) => {
    ...
    const idShort = getPublicOrderNumber(order) || String(order?.id || order?._id || '').slice(-8);
