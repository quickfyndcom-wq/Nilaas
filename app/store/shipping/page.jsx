...
export default function StoreShippingSettings() {
  ...
  const [form, setForm] = useState({
    ...
    regionalDeliveryFee: '',
    estimatedDays: '1-3',
    methodTitleFree: 'Free shipping',
    methodTitlePaid: 'Standard shipping',
    deliverySubtitle: 'Delivered within {days} days',
    freePriceLabel: 'FREE',
    enableCOD: true,
    codFee: 0,
      ...
      try {
        ...
        if (data?.setting) {
          setForm({
            ...
            estimatedDays: data.setting.estimatedDays || '1-3',
            methodTitleFree: data.setting.methodTitleFree || 'Free shipping',
            methodTitlePaid: data.setting.methodTitlePaid || 'Standard shipping',
            deliverySubtitle: data.setting.deliverySubtitle || 'Delivered within {days} days',
            freePriceLabel: data.setting.freePriceLabel || 'FREE',
  ...
  return (
    <div className='p-6 max-w-4xl'>
      ...
      <div className='space-y-6'>
        {/* Checkout display — what customers see */}
        <div className='bg-white p-6 rounded-xl border border-slate-200'>
          <h2 className='text-xl font-semibold text-slate-800 mb-1'>Checkout display</h2>
          <p className='text-sm text-slate-500 mb-5'>
            Edit the shipping method name, delivery time text, and FREE label shown on checkout.
          </p>

          <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
            <div>
              <label className='block text-sm font-medium text-slate-700 mb-2'>Title when shipping is free</label>
              <input
                type='text'
                value={form.methodTitleFree}
                onChange={(e) => setForm((s) => ({ ...s, methodTitleFree: e.target.value }))}
                className='w-full border border-slate-300 rounded px-3 py-2'
                placeholder='Free shipping'
              />
            </div>
            <div>
              <label className='block text-sm font-medium text-slate-700 mb-2'>Title when shipping is paid</label>
              <input
                type='text'
                value={form.methodTitlePaid}
                onChange={(e) => setForm((s) => ({ ...s, methodTitlePaid: e.target.value }))}
                className='w-full border border-slate-300 rounded px-3 py-2'
            ...
            <div>
              ...
              <input
                ...
                placeholder='1-3'
              />
              <p className='text-xs text-slate-500 mt-1'>Used where you write {'{days}'} below</p>
            </div>
            <div>
              <label className='block text-sm font-medium text-slate-700 mb-2'>Free price label</label>
              <input
                type='text'
                value={form.freePriceLabel}
                onChange={(e) => setForm((s) => ({ ...s, freePriceLabel: e.target.value }))}
                className='w-full border border-slate-300 rounded px-3 py-2'
                placeholder='FREE'
              />
            </div>
            <div className='md:col-span-2'>
              <label className='block text-sm font-medium text-slate-700 mb-2'>Delivery subtitle</label>
              <input
                type='text'
                value={form.deliverySubtitle}
                onChange={(e) => setForm((s) => ({ ...s, deliverySubtitle: e.target.value }))}
                className='w-full border border-slate-300 rounded px-3 py-2'
                placeholder='Delivered within {days} days'
              />
              <p className='text-xs text-slate-500 mt-1'>
                Use <code className='bg-slate-100 px-1'>{'{days}'}</code> to insert estimated days
              </p>
            </div>
          </div>

          <div className='border border-[#2a1210] bg-[#fafafa] p-4 flex items-center justify-between gap-4'>
            <div>
              <p className='text-[10px] uppercase tracking-widest text-slate-400 mb-1'>Preview</p>
              <p className='font-medium text-[#2a1210]'>{form.methodTitleFree || 'Free shipping'}</p>
              <p className='text-sm text-[#6e5048] mt-0.5'>
                {(form.deliverySubtitle || 'Delivered within {days} days').replace(
                  /\{days\}/gi,
                  form.estimatedDays || '1-3'
                )}
              </p>
            </div>
            <p className='font-semibold text-[#2a1210] shrink-0'>{form.freePriceLabel || 'FREE'}</p>
          </div>
        </div>
