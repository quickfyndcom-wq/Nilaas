    ...
    try {
        ...
        if (paymentMethod && !Object.values(PaymentMethod).includes(paymentMethod)) {
            return NextResponse.json({
                ...
                allowed: Object.values(PaymentMethod),
            }, { status: 400 });
        }
        ...
        for (const [storeId, sellerItems] of ordersByStore.entries()) {
            ...
            // Email after COD immediately; Razorpay emails after payment verify
                ...
                try {
                    ...
                    if (customerEmail) {
                        await sendOrderConfirmationEmail({
                            ...
                            shippingAddress: order.shippingAddress || populatedOrder?.shippingAddress,
        ...
        // Razorpay — create gateway order; cart cleared after verify
        if (paymentMethod === PaymentMethod.RAZORPAY) {
            ...
            const amountPaise = Math.round(fullAmount * 100);
            if (amountPaise < 100) {
                return NextResponse.json({ error: 'Order amount too low for online payment' }, { status: 400 });
            }

            const receipt = `nilaas_${orderIds[0]?.slice(-10) || Date.now()}`;
        ...
        // Clear cart for logged-in users (COD)
        if (userId) {
